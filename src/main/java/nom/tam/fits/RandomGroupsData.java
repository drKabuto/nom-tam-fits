...
public class RandomGroupsData extends Data {
    ...
    protected void fillHeader(Header h) throws FitsException {
        ...
        h.addValue(NAXISn.n(1).card().value(0).useDefaultComment());
        for (int i = 2; i <= ddims.length + 1; i += 1) {
            h.addValue(NAXISn.n(i).card().value(ddims[i - 2]).useDefaultComment());
        }

        h.addValue(GROUPS.card().value(true).useDefaultComment());
        h.addValue(GCOUNT.card().value(dataArray.length).useDefaultComment());
        h.addValue(PCOUNT.card().value(pdims[0]).useDefaultComment());
    }
